...
from basic import isoDateTimeFromMilli, convertProdState, convertPriority
...
class EventCompatInfo(object):
    ...
    def __init__(self, event_summary):
    ...
    @property
    def Location(self):
        return self._lookupDetailPath('/zport/dmd/Locations', self._eventDetails.get('zenoss.device.location'))

    @property
    def DeviceGroups(self):
        return self._lookupDetailPath('/zport/dmd/Groups', self._eventDetails.get('zenoss.device.groups'))

    @property
    def Systems(self):
        return self._lookupDetailPath('/zport/dmd/Systems', self._eventDetails.get('zenoss.device.systems'))

    @property
    def DeviceClass(self):
        return self._lookupDetailPath('/zport/dmd/Devices', self._eventDetails.get('zenoss.device.device_class'))
    ...
    def prodState(self):
        ...
        if prodState is not None:
            return convertProdState(prodState)
    ...
    def DevicePriority(self):
        ...
        if DevicePriority is not None:
            return convertPriority(DevicePriority)
    ...
    def _uuidUrl(self, uuid):
        if uuid:
            return 'http://10.103.11.40:9089/zport/dmd/goto?guid=%s' % uuid
    ...
    def _getPathFromUuid(self, uuid):
        if uuid:
            path = self._manager.getPathFromUuid(uuid)
            if path:
    ...
    def _getNameFromUuid(self, uuid):
        ...
        if uuid:
            ...
            if path:
                name = self._manager.getNameFromPath(path)
                return name
